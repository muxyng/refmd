        ...
        <div className="flex-1 overflow-hidden">
          ...
          {!error && (
            <div className="flex h-full min-w-0">
              <div className="w-[30%] min-w-[240px] max-w-[360px] flex flex-col h-full min-h-0">
                ...
                <div className="flex-1 overflow-y-auto min-w-0">
                  <div className="p-4 space-y-3 min-w-0">
                    ...
                    {!isLoading && commits.map((c) => (
                      <div
                        key={c.hash}
                        className={cn('border rounded-lg p-4 cursor-pointer transition-colors overflow-hidden', selectedCommit?.hash === c.hash ? 'bg-accent border-accent-foreground/20' : 'hover:bg-accent/50')}
                      ...
                      >
                        <div className="flex items-start gap-2">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="font-medium text-sm block break-words">{(c.message || '').split('\n')[0] || '(no message)'}</div>
                            <div className="text-xs text-muted-foreground flex items-center gap-2 min-w-0">
                              <span className="flex items-center gap-1 break-words"><User className="h-3 w-3" />{c.author_name} &lt;{c.author_email}&gt;</span>
                            </div>
                          </div>
                        </div>
                      </div>
